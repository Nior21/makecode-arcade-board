...
async function ensureSshReady() {
  ...
  if (listen.stdout.trim() === 'yes') {
    ...
    return { ok: true, user: user.stdout || 'user', port: SSH_PORT, steps: [{ step: 'already_running', ok: true }] };
  }
  ...
  if (listen.stdout.trim() !== 'yes') {
    const diag = await runShell('sshd -t 2>&1; pgrep -af sshd 2>/dev/null | head -3');
    steps.push({ step: 'sshd_diag', ok: false, detail: (diag.stdout || diag.stderr).slice(0, 400) });
  }
  const user = await runShell('whoami');
  return {
...
async function setTempPassword(user, password) {
  ...
  const script = [
    ...
    `  printf '%s:%s\\n' '${safeUser}' "$P" | chpasswd 2>&1 && echo ok && exit 0`,
    `fi`,
    `if (printf '%s\\n' "$P" "$P") | passwd 2>&1; then`,
    `  echo ok`,
    `  exit 0`,
  ...
  const ok = /\bok\b/.test(out) || /successfully set|password updated|updated successfully|all authentication tokens updated/i.test(out);
  return { ...result, ok };
}
