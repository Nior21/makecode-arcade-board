...
function linkBoard({ owner, repo, branch }) {
  if (!owner || !repo) throw new Error('Нужны owner и repo');
  const link = { owner: String(owner).trim(), repo: String(repo).trim(), branch: branch || 'main' };
  writeBoardLink(link);
